...
/**
 * 〈Oauth2 token 服务实现类重写〉
 *
 * @author roy
 * @create 2021/11/26
 * @since 1.0.0
 */
public class OverrideTokenServices
    implements AuthorizationServerTokenServices,
        ResourceServerTokenServices,
        ConsumerTokenServices,
        InitializingBean {

  private final Logger logger = LoggerFactory.getLogger(OverrideTokenServices.class);
  ...
  public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication)
    ...
    logger.info("######## existingAccessToken:{}", existingAccessToken);
    ...
    if (existingAccessToken != null) {
      ...
      try {
        // 此处为实现全局唯一登录，移除原有token，将token唯一标识jit 放入redis为网关提供判断依据
        refreshToken = existingAccessToken.getRefreshToken();
        tokenStore.removeRefreshToken(refreshToken);
        tokenStore.removeAccessToken(existingAccessToken);
        JWSObject jwsObject = JWSObject.parse(existingAccessToken.getValue());
        ...
        redisService.set(
            AuthConstant.TOKEN_BLACKLIST_PREFIX + jti, null, (exp - currentTimeSeconds));
  ...
  public OAuth2AccessToken refreshAccessToken(String refreshTokenValue, TokenRequest tokenRequest)
    ...
    if (this.authenticationManager != null && !authentication.isClientOnly()) {
      // The client has already been authenticated, but the user authentication might be old now, so
      // give it a
  ...
  private OAuth2RefreshToken createRefreshToken(OAuth2Authentication authentication) {
    ...
    if (validitySeconds > 0) {
      return new DefaultExpiringOAuth2RefreshToken(
          value, new Date(System.currentTimeMillis() + (validitySeconds * 1000L)));
    }
  ...
  /**
   * The validity (in seconds) of the refresh token. If less than or equal to zero then the tokens
  ...
  /**
   * The default validity (in seconds) of the access token. Zero or negative for non-expiring
   * tokens. If a client details service is set the validity period will be read from the client,
   * defaulting to this value if not defined by the client.
   *
  ...
  /**
   * An authentication manager that will be used (if provided) to check the user authentication when
  ...
  /**
   * The client details service to use for looking up clients (if necessary). Optional if the access
   * token expiry is set globally via {@link #setAccessTokenValiditySeconds(int)}.
   *
