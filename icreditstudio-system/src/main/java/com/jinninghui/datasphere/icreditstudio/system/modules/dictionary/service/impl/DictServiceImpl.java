...
import com.jinninghui.datasphere.icreditstudio.system.modules.dictionary.dto.RedisDictColumnDTO;
...
import com.jinninghui.datasphere.icreditstudio.system.modules.dictionary.mapper.DictMapper;
...
public class DictServiceImpl extends ServiceImpl<DictMapper, DictEntity> implements DictService {
    ...
    public BusinessResult<Boolean> save(DictSaveParam param) {
        ...
        if (null != oldDict) {
            throw new AppException(DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000091.code, DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000091.message);
        }
    ...
    private static void checkDictParam(DictSaveParam param) {
        if (StringUtils.isEmpty(param.getWorkspaceId())) {
            throw new AppException(DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000000.code, DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000000.message);
        }
        if (!param.getEnglishName().matches("[a-zA-Z_]{0,50}")) {
            throw new AppException(DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000081.code, DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000081.message);
        }
        if (!param.getChineseName().matches("[\u4e00-\u9fa5]{0,50}")) {
            throw new AppException(DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000082.code, DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000082.message);
        }
        if (!StringUtils.isEmpty(param.getDictDesc()) && param.getDictDesc().length() > 250) {
            throw new AppException(DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000083.code, DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000083.message);
        }
    }

    private void checkDictId(String id) {
        if (StringUtils.isEmpty(id)) {
            throw new AppException(DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000080.code, DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000080.message);
        }
    }
    ...
    public BusinessResult<Boolean> importDict(MultipartFile file, String dictSaveRequestJson) {
        if (file.isEmpty()) {
            throw new AppException(DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000079.code, DictResourceCodeBean.ResourceCode.RESOURCE_CODE_60000079.message);
        }
