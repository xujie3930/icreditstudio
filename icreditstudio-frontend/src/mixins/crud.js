...
import { deepClone, upperCaseFirst } from '@/utils/util'
...
export default {
  ...
  methods: {
    ...
    mixinFilterTableOperationByPermission(data) {
      ...
      // 有则通过权限过滤
      _operation.operationList = _operation.operationList
        .filter(e => this.permissions.includes(e.key))
    },
    /* 获取表格数据 */
    ...
    mixinHandleCreateOrUpdate(row) {
      if (this.validForm(this.mixinDialogFormConfig.refName)) {
        ...
        request
          .then(res => {
            if (res.success) {
              if (this[`interceptorsResponse${upperCaseFirst(_type)}`](res.data)) return
              this.$notify.success({
                title: '成功',
