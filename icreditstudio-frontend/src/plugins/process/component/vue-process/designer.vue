  ...
  computed: {
    ...
    myMenuOptions() {
      ...
      if (this.ctxMenuOptions) {
        let defaultShow = {
          ...ctxMenuOptions.defaultMenu,
          ...this.ctxMenuOptions.defaultMenu
        }
        if (defaultShow.copy) {
          defaultMenu.push({
            icon: 'fuzhi',
            text: this.t('vue-process.copy'),
            value: 'copy'
          })
        }
        if (defaultShow.delete) {
          defaultMenu.push({
            icon: 'shanchu',
            text: this.t('vue-process.delete'),
            value: 'delete'
          })
        }
        if (defaultShow.param) {
          defaultMenu.push({
            icon: 'canshu',
            text: this.t('vue-process.Parameter'),
            value: 'param'
          })
        }
  ...
  watch: {
    viewOptions(v) {
      ...
      if (shapeView !== this.showViews.shapeView) {
        commit(this.$store, 'UPDATE_SHAPE_OPTIONS', {
          viewWidth: this.showViews.shapeView ? 250 : 0
        })
  ...
  methods: {
    ...
    addNode(e) {
      ...
      let node = Object.assign(
        ...
        this.state.nodeOptions,
        {
          ...
          x:
            (e.pageX -
              this.state.baseOptions.nodeViewOffsetX -
              (this.state.nodeOptions.width * this.state.baseOptions.pageSize) /
                2) /
              this.state.baseOptions.pageSize +
            this.nodeViewScrollLeft / this.state.baseOptions.pageSize,
          y:
            (e.pageY -
              this.state.baseOptions.nodeViewOffsetY -
              (this.state.nodeOptions.height *
                this.state.baseOptions.pageSize) /
                2) /
              this.state.baseOptions.pageSize +
            this.nodeViewScrollTop / this.state.baseOptions.pageSize
        }
      )
    ...
    moveShape(e) {
      ...
      if (this.state.draging.type == 'shape') {
        ...
        // 超出临界
        if (
          e.pageX - this.state.baseOptions.shapeViewOffsetX >
          this.state.shapeOptions.viewWidth - this.state.shapeOptions.width / 2
        ) {
    ...
    stopMoveShape(e) {
      ...
      if (this.state.draging.type == 'shape' && this.state.nodes.length > 0) {
        ...
        if (
          e.pageX - this.state.baseOptions.shapeViewOffsetX >
          this.state.shapeOptions.viewWidth
        ) {
          // 超出临界，保存节点
          commit(this.$store, 'UPDATE_NODE', {
            key: lasNode.key,
