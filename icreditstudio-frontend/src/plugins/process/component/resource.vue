      ...
      <Upload
        ...
        <div style="padding: 20px 0">
          <Icon
            ...
            size="52"
            style="color: #3399ff"/>
          <p class="el-upload__text">{{$t('message.process.resourceBar.TZDLSC')}}</p>
        </div>
      </Upload>
    </div>
    <Spin
...
import { NODETYPE } from '../service/nodeType'
export default {
  props: {
    resources: {
      type: Array,
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      if (this.isRipetition) {
        if (this.nodeType === NODETYPE.SPARKPY) {
          if (!(/\.zip$/i.test(file.name))) {
            this.$Message.warning(this.$t('message.process.resourceBar.PYSPARKNODE'));
            return false;
          }
        } else if (this.nodeType === NODETYPE.SPARKSQL) {
          if (!(/\.jar$/i.test(file.name))) {
            this.$Message.warning(this.$t('message.process.resourceBar.SPARKNOSW'));
            return false;
          }
        }
      ...
      if (isInFlag && !this.isRipetition) {
        this.$Message.warning(this.$t('message.process.resourceBar.WJYCZ'));
        return false;
      } else if (!regLeaf.test(file.name)) {
        this.$Message.warning(this.$t('message.process.resourceBar.WJMCBHF'));
        return false;
      } else if (sizeResult) {
        this.$Message.warning(this.$t('message.process.resourceBar.SCBCG100'));
        return false;
      }
    },
    // 成功时的回调
    handleSuccess(response, file, fileList) {
      if (response.status === 0) {
        this.$Message.success(this.$t('message.process.resourceBar.SCCG', {name: file.name}));
    ...
    exceededSize(file, fileList) {
      this.$Message.warning(this.$t('message.process.resourceBar.WJCCXE'));
    },
    handleRemove(file, fileList) {
      api.fetch('/bml/deleteResource', {
        ...
        this.$emit('update-resources', this.uploadFiles);
        this.$Message.success(this.$t('message.process.resourceBar.WJBSC', {name: file.name}));
      });
    },
