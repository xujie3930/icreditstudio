<template>
  <Modal
    ...
    <Form
      ...
      <FormItem
        :label="$t('message.functionSetting.HSM')"
        prop="name">
        <Input v-model="setting.name"></Input>
      </FormItem>
      <FormItem
        :label="$t('message.functionSetting.HSLX')"
        prop="fnType">
        <RadioGroup
          ...
          <Radio
            v-if="isUdf"
            :label="$t('message.functionSetting.TY')"/>
          <Radio
            v-if="isUdf"
            label="spark"/>
          <Radio
            v-if="!isUdf"
            :label="$t('message.functionSetting.ZDYHS')"/>
        </RadioGroup>
      </FormItem>
      <FormItem
        v-if="fnCategory.isCommon"
        :label="$t('message.functionSetting.B')"
        prop="jarPath">
        <directory-dialog
          :tree="tree"
      ...
      <FormItem
        ...
        :label="$t('message.functionSetting.JBLU')"
        prop="sparkPath">
        <directory-dialog
          :tree="tree"
          :load-data-fn="loadDataFn"
      ...
      <FormItem
        ...
        :label="$t('message.functionSetting.ZCGS')"
        prop="jarPara">
        <Input
          v-model="setting.jarPara"
          :placeholder="$t('message.functionSetting.ZCHS')"></Input>
      </FormItem>
      <FormItem
        ...
        <div class="format-div">
          <FormItem prop="scalaTypeL">
            <Input
              ...
              :placeholder="$t('message.functionSetting.FHLX')"
              class="format-input"
              @input="resetRF"></Input>
          </FormItem>
          <FormItem prop="scalaTypeR">
            <Input
              ...
              :placeholder="$t('message.functionSetting.CSLX')"
              class="format-input"
              @input="resetRF"></Input>
          </FormItem>
      ...
      <FormItem
        ...
        :label="$t('message.functionSetting.YLLX')">
      ...
      <FormItem
        :label="$t('message.functionSetting.SYGS')"
        required>
        <div class="format-div">
          ...
          <FormItem class="format-item">
            <Input
              v-if="fnType===2"
              v-model="setting.scalaTypeR"
              :placeholder="$t('message.functionSetting.SRLX')"
              disabled></Input>
            <Input
              v-else
              v-model="setting.useFormatParaR"
              :placeholder="$t('message.functionSetting.SRLX')"></Input>
          </FormItem>
        </div>
      </FormItem>
      <FormItem :label="$t('message.functionSetting.YLLX')">
        <use-preview
          :name="setting.name"
      ...
      <FormItem :label="$t('message.functionSetting.HSMS')">
        <Input
    ...
    <template slot="footer">
      <Checkbox
        ...
        class="fn-default-load">{{ $t('message.functionSetting.MRJZ') }}</Checkbox>
      <Button
        :loading="loading"
        type="primary"
        class="fn-sure-btn"
...
import directoryDialog from '../directoryDialog/index.vue';
...
export default {
  ...
  data() {
    return {
      ...
      show: false,
      title: this.$t('message.functionSetting.XZHS'),
      btnLabel: this.$t('message.functionSetting.LJCJ'),
      modalHeight: '',
      // 1是修改，0是新增)
      model: 0,
      node: null,
      ...
      setting: {
        name: '',
        fnType: this.$t('message.functionSetting.TY'),
        jarPath: '',
        sparkPath: '',
        customPath: '',
        useFormatParaL: '',
      ...
      rules: {
        name: [
          { required: true, message: this.$t('message.functionSetting.QSRMC'), trigger: 'blur' },
          {
            min: 1,
            max: 100,
            message: this.$t('message.functionSetting.CDZ'),
            trigger: 'change',
          },
          {
            type: 'string',
            ...
            message:
                            this.$t('message.functionSetting.BXYZMKT'),
            trigger: 'change',
          },
        ],
        fnType: [
          {
            required: true,
            message: this.$t('message.functionSetting.QZSXZYZLX'),
            trigger: 'change',
          },
        ],
        jarPath: [
          {
            required: true,
            message: this.$t('message.functionSetting.QSRWZLJ'),
            trigger: 'change',
          },
          // $t('message.functionSetting.KHDBJZPP')
          {
            type: 'string',
            pattern: /^[\w\u4e00-\u9fa5:.\\/]*(jar)$/,
            message: this.$t('message.functionSetting.HZMZC'),
            trigger: 'change',
          },
        ],
        sparkPath: [
          ...
          // $t('message.functionSetting.KHDBJZPP')
          {
            ...
            message: this.$t('message.functionSetting.ZCPYSCA'),
            trigger: 'change',
          },
        ],
        customPath: [
        ...
        jarPara: [
          {
            required: true,
            message: this.$t('message.functionSetting.SRZCHS'),
            trigger: 'blur',
          },
        ],
        pyPara: [
        ...
        scalaTypeL: [
          {
            type: 'string',
            required: true,
            message: this.$t('message.functionSetting.SRFHLX'),
            trigger: 'blur',
          },
        ],
        scalaTypeR: [
  ...
  watch: {
    ...
    'setting.fnType'(val) {
      let map = {};
      map[this.$t('message.functionSetting.TY')] = 'isCommon';
      map['spark'] = 'isSpark';
      map[this.$t('message.functionSetting.ZDYHS')] = 'isCustom';
      let type = map[val];
  ...
  methods: {
    open({ node }) {
      this.resetData();
      const titleMap = [this.$t('message.functionSetting.XZHS'), this.$t('message.functionSetting.XGHS')];
      const btnLabelMap = [this.$t('message.functionSetting.LJCJ'), this.$t('message.functionSetting.XG')];
      this.show = true;
      this.node = node.data;
      if (this.node.isLeaf) {
        ...
        this.model = 0;
        this.setting.fnType = this.isUdf ? this.$t('message.functionSetting.TY') : this.$t('message.functionSetting.ZDYHS');
      }
    ...
    init() {
      ...
      if (udfType === 0) {
        ...
        fnType = this.$t('message.functionSetting.ZDYHS');
      }
      if (registerFormat) {
