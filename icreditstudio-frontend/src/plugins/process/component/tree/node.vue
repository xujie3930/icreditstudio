<template>
  ...
  >
    ...
    >
      <span v-if="!node.isLeaf" class="we-tree-node__expand-icon" />
      <span :class="supportIcon(node)" class="we-tree-node__icon" />
      <template v-if="node.isEditState">
        <div :class="{ 'is-error': !inputValid }" class="we-tree-node__edit">
          <input
            ref="edit"
            :value="node.label"
            type="text"
          ...
          />
          <div v-if="errorMsg" class="we-tree-node__error">{{ errorMsg }}</div>
        </div>
      </template>
      <template v-else-if="node.data && node.data.isFn">
        <div class="we-tree-node__checkbox">
          <input
            ...
            :disabled="
              node.data.disabled ||
                (node.data.type === 'self' && node.data.shared)
            "
            type="checkbox"
            @change="hanlderCheck"
          />
          <view-modal :node="node.data">
            <span
              ...
              :class="{ expired: node.data.expire }"
              >{{ node.data.name }}</span
            >
          </view-modal>
        </div>
    ...
    <div v-show="expanded" class="we-tree-node__children">
      <we-tree-node
...
import viewModal from '../../component/functionView'
export default {
  name: 'WeTreeNode',
  components: {
    viewModal
  ...
  methods: {
    ...
    supportIcon(node) {
      ...
      if (node.isLeaf && match) {
        return `is-leaf ${match.logo}`
      } else if (node.isLeaf && !match && !node.data.isFn) {
        return 'is-leaf fi-file'
      } else if (!node.isLeaf) {
        return 'fi-folder'
