<template>
  <div class="process-module">
    ...
    >
      <template v-if="!myReadonly && type === 'flow'">
        ...
        <div
          ...
          @click="workflowRun"
        >
          <span class="icon fi-play" style="padding-top:2px;"></span>
    ...
    >
      <div class="process-module-param-modal-header">
        <h3>
          {{
            isResourceShow
              ? $t('message.process.resource')
              : $t('message.process.params')
          }}{{ $t('message.process.seting') }}
        </h3>
      </div>
    ...
    <Modal width="450" v-model="saveModal">
      <div class="process-module-title" slot="header">
        {{ $t('message.process.save') }}
      </div>
      <Form ref="formSave" :model="saveModel" :label-width="85">
        <FormItem
          :label="$t('message.newConst.comment')"
          prop="comment"
          :rules="[
            { required: true, message: $t('message.process.inputComment') },
            { message: $t('message.process.commentLengthLimit'), max: 255 }
          ]"
        >
          <Input
    ...
    <Modal v-model="importModal">
      ...
      <div v-show="importModel.names.length">
        <div v-for="filename in importModel.names" :key="filename">
          {{ filename }}
        </div>
      </div>
      <div slot="footer">
        <Button type="primary" @click="importJSON">{{
          $t('message.process.confirmImport')
        }}</Button>
      </div>
    </Modal>
    ...
    >
      ...
      <div slot="footer">
        <Button type="primary" @click="repetitionName">{{
          $t('message.newConst.ok')
        }}</Button>
      </div>
    </Modal>
    <associate-script ref="associateScript" @click="associateScript" />
    <Modal :title="addNodeTitle" v-model="addNodeShow">
      <Form
        label-position="left"
        :label-width="110"
        ref="addFlowfoForm"
      ...
      >
        <FormItem
          ...
          :rules="[
            {
              required: true,
              message: $t('message.process.inputNodeName'),
              trigger: 'blur'
            },
            {
              type: 'string',
      ...
      <div slot="footer">
        <Button type="text" size="large" @click="addFlowCancel">
          {{ $t('message.newConst.cancel') }}
        </Button>
        <Button type="primary" size="large" @click="addFlowOk">
          {{ $t('message.newConst.ok') }}
        </Button>
      </div>
    </Modal>
    <template v-for="item in json && json.nodes">
      <console
        ...
        :style="{
          left: shapeWidth + 'px',
          width: `calc(100% - ${shapeWidth}px)`
        }"
        @close-console="closeConsole(item)"
      ></console>
    </template>
  </div>
</template>
...
import vueProcess from './component/vue-process'
...
import { NODETYPE, ext } from './service/nodeType'
...
export default {
  ...
  mounted() {
    ...
    if (!this.myReadonly) {
      this.timer = setInterval(() => {
        if (this.jsonChange && !this.nodebaseinfoShow) {
          // paramsIsChange参数是否在编辑操作
  ...
  methods: {
      ...
      api.fetch(`/dss/listNodeType${params}`, 'get').then(res => {
        ...
        this.shapes = this.shapes.map(item => {
          item.children = item.children.filter(subItem =>
            nodeArray.some(nodeType => nodeType.nodeType === subItem.type)
          )
          return item
        })
      })
    ...
    initNode(arg) {
      ...
      if (
        [NODETYPE.DISPLAY, NODETYPE.DASHBOARD].includes(
          this.clickCurrentNode.type
        )
      ) {
        // 如果是已经创建过的，一定是在jobContent里保存有值得
        if (
          !this.clickCurrentNode.jobContent ||
          !this.clickCurrentNode.jobContent.id
        ) {
      ...
      if (
        this.clickCurrentNode.jobContent &&
        this.clickCurrentNode.jobContent.jobParams
      ) {
        ...
        this.clickCurrentNode.jobContent.jobParams = defaultValueAction(
          this.clickCurrentNode.jobContent.jobParams
        )
      }
      if (
        ...
        if (this.clickCurrentNode.type === NODETYPE.EVENTCHECKERW) {
          ...
          [NODETYPE.SPARKSQL, NODETYPE.SPARKPY, NODETYPE.SCALA].includes(
            this.clickCurrentNode.type
          )
        ) {
              ...
              if (settings.length > 0) {
                settings.forEach(item => {
                  if (item.key === 'spark.driver.cores') {
                    jobParams['spark-driver-memory'] =
                      +item.value || +item.defaultValue
                  } else if (item.key === 'spark.executor.memory') {
                    jobParams['spark-executor-memory'] =
                      +item.value || +item.defaultValue
                  } else if (item.key === 'spark.executor.cores') {
                    jobParams['spark-executor-cores'] =
                      +item.value || +item.defaultValue
                  } else if (item.key === 'spark.executor.instances') {
                    jobParams['spark-executor-instances'] =
                      +item.value || +item.defaultValue
                  }
                })
                jobParams['wds-linkis-yarnqueue'] =
                  rst[1].fullTree[0].settings[0].value ||
      ...
      if (
        !this.clickCurrentNode.params ||
        isEmpty(this.clickCurrentNode.params.configuration)
      ) {
    ...
    saveNodeBaseInfo(arg) {
      ...
      if (node.type == NODETYPE.FLOW) {
        ...
        if (!node.title.match(reg)) {
          return this.$Message.warning(
            this.$t('message.newConst.validNameDesc')
          )
        }
        if (!node.jobContent.embeddedFlowId) {
          ...
          this.ops = this.ops.map(item => {
            if (
              item.subsubFlowID === node.jobContent.embeddedFlowId &&
              item.op == 'update'
            ) {
      ...
      if (!flage)
        return this.$Message.success(
          this.$t('message.process.saveParamsNotice')
        )
    },
    saveNodeParam(...arg) {
    ...
    handleUpload(file) {
      ...
      if (sizeResult)
        return this.$Message.warning(this.$t('message.process.JsonLimit'))
      ...
      Promise.all(fp).then(res => {
        ...
        res.forEach(fileItem => {
          ...
          try {
            ...
            valid = false
            return this.$Message.warning(
              `${this.$t('message.process.jsonFormat')}：${error.message}`
            )
          }
          ...
          for (let i = 0; i < json.edges.length; i++) {
            ...
            if (
              ...
              return this.$Message.warning(
                this.$t('message.process.jsonEdgeNotice')
              )
            }
          }
          ...
          for (let i = 0; i < json.nodes.length; i++) {
            ...
            if (!node.layout || !node.jobType || !node.id) {
              valid = false
              return this.$Message.warning(
                this.$t('message.process.jsonNodeNotice')
              )
            } else if (node.layout) {
              ...
              if (
                ...
                valid = false
                return this.$Message.warning(
                  this.$t('message.process.jsonLayoutNotice')
                )
              }
            }
          }
          ...
          this.shapes.forEach(it => {
            if (it.children) {
              validTypes = validTypes.concat(
                it.children.map(child => child.type).filter(t => t)
              )
            }
          })
          ...
          if (json.edges && !Array.isArray(json.edges)) {
            valid = false
            return this.$Message.warning(
              this.$t('message.process.jsonEdgesNoArray')
            )
          }
          if (json.nodes && !Array.isArray(json.nodes)) {
            valid = false
            return this.$Message.warning(
              this.$t('message.process.jsonNodesNoArray')
            )
          }
          ...
          if (
            invalidNodes.length === json.nodes.length ||
            json.nodes.length < 1
          ) {
            valid = false
            return this.$Message.warning(
              `${this.$t('message.process.jsonNoType')}（${validTypes.join(
                '、'
              )}）`
            )
          } else {
            invalidNodes.forEach(node =>
              console.warn(
                `${this.$t('message.process.nosupprotType')}${node.type}`,
                node
              )
            )
          }
    ...
    changeJsonData(json) {
      ...
      if (this.importReplace) {
        ...
        if (Array.isArray(this.json.edges) && Array.isArray(json.edges)) {
          ...
          json.edges = this.json.edges.concat(json.edges).map(item => {
            // 修改空节点名称，避免全局工程下节点同名，不是很严谨，但能减少使用人员去修改重名
      ...
      const emptyNode = json.nodes
        .filter(
          item =>
            ...
            (!item.jobContent || !item.jobContent.jobParams)
        )
        .map(item => item.title)

      const isFiveNode = json.nodes.filter(item => {
        return (
          !item.jobContent && item.jobType === NODETYPE.FLOW && this.rank >= 4
        )
      })
      if (isFiveNode.length > 0)
        return this.$Message.warning(this.$t('message.process.deleteNodeSave'))
      if (emptyNode.length > 0 && !f) {
        // 自动保存时不用提示
        this.$Modal.confirm({
          title: this.$t('message.process.saveAbnormal'),
          content: this.$t('message.process.emptyNodeHtml', {
            emptyNode: emptyNode.join(',')
          }),
    ...
    saveRequest(json, comment, f, cb) {
      ...
      paramsJson.nodes = paramsJson.nodes.map(node => {
        if (node.jobContent && node.jobContent.jobParams) {
          // 保存数据前添加默认值
          node.jobContent.jobParams = defaultValueAction(
            node.jobContent.jobParams
          )
        }
        return node
      })
    ...
    handleOutsideClick(e) {
      ...
      if (
        (paramButton && paramButton.contains(e.target)) ||
        e.target === paramButton ||
        (resourceButton && resourceButton.contains(e.target)) ||
        e.target === resourceButton
      ) {
        return
      }
    ...
    handleOutsideClickNode(e) {
      if (
        e.target.className === 'node-box-content' ||
        e.target.className === 'node-box'
      )
        return
    ...
    nodeDelete(node) {
      ...
      if (
        ...
        node.jobContent &&
        node.jobContent.embeddedFlowId
      ) {
        // 不用调用删除接口改为记录
        this.ops.push({
          id: node.jobContent.embeddedFlowId,
      ...
      if (
        [NODETYPE.DISPLAY, NODETYPE.DASHBOARD].includes(node.type) &&
        node.jobContent &&
        node.jobContent.id
      ) {
        let id =
          NODETYPE.DASHBOARD == node.type
            ? node.jobContent.dashboardId
            : node.jobContent.id
        this.ops.push({
          id,
          nodeType: node.type,
    ...
    saveNode(node) {
      // 保存节点参数配置
      if (this.myReadonly)
        return this.$Message.warning(this.$t('message.process.readonly'))
      this.saveNodeBaseInfo(node)
    },
    checkAssociated(node) {
      if (this.myReadonly)
        return this.$Message.warning(
          this.$t('message.process.readonlyNoAssociated')
        )
      if (
        [
          NODETYPE.SPARKSQL,
    ...
    associateScript(node, path, cb) {
      api
        ...
        .then(rst => {
          ...
          api
            .fetch('/filesystem/saveScriptToBML', params, 'post')
            .then(res => {
              ...
              this.$emit('check-opened', node, isOpened => {
                ...
                if (isOpened) {
                  api
                    .fetch('/filesystem/openScriptFromBML', params, 'get')
                    .then(res => {
    ...
    addFlowOk() {
      this.$refs.addFlowfoForm.validate(valid => {
        if (valid) {
          ...
          if (this.myReadonly)
            return this.$Message.warning(
              this.$t('message.process.readonlyNoCeated')
            )
    ...
    relySelect(node) {
      ...
      this.json.nodes = this.json.nodes.map(subItem => {
        if (
          stepArray.some(item => item.target === subItem.key) ||
          subItem.key === node.key
        ) {
    ...
    copyNode(node) {
      node = this.bindNodeBasicInfo(node)
      if (node.enable_copy === false)
        return this.$Message.warning(this.$t('message.process.noCopy'))
      if (this.myReadonly)
        return this.$Message.warning(this.$t('message.process.readonlyNoCopy'))
      this.cacheNode = JSON.parse(JSON.stringify(node))
    },
    pasteNode(e) {
      if (this.myReadonly)
        return this.$Message.warning(this.$t('message.process.noPaste'))
      if (!this.cacheNode) {
      ...
      if (
        [NODETYPE.DISPLAY, NODETYPE.DASHBOARD, NODETYPE.FLOW].includes(
          this.cacheNode.type
        )
      ) {
    ...
    allDelete() {
      if (this.myReadonly)
        return this.$Message.warning(this.$t('message.process.noDelete'))
      const selectNodes = this.$refs.process
        .getSelectedNodes()
        .map(item => item.key)
    ...
    openConsole(node) {
      ...
      this.shapeWidth =
        this.$refs.process && this.$refs.process.state.shapeOptions.viewWidth
    },
    closeConsole(node) {
      this.openningNode = null
    },
    ...
    workflowStop() {
      clearTimeout(this.excuteTimer)
      api
        .fetch(`/entrance/${this.workflowExeteId}/kill`, 'get')
        .then(res => {
          this.workflowIsExecutor = false
      ...
      api.fetch(`/entrance/${execID}/execution`, {}, 'get').then(res => {
        ...
        const actionStatus = {
          ...
          runningJobs: {
            ...
            status: 1,
            iconType: 'ios-loading',
            colorClass: { 'executor-loading': true },
            isShowTime: true,
            title: '执行中',
            showConsole: true
          },
          succeedJobs: {
            ...
            colorClass: { 'executor-success': true },
            isShowTime: false,
            title: '执行成功',
            showConsole: true
          },
          failedJobs: {
            ...
            colorClass: { 'executor-faile': true },
            isShowTime: false,
            title: '执行失败',
            showConsole: true
          },
    ...
    timeTransition(time) {
      ...
      const timeResult = `${addZero(hour)}:${addZero(minute)}:${addZero(
        second
      )}`
      return timeResult
    },
