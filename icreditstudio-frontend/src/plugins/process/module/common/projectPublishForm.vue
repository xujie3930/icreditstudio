<template>
  <Form :label-width="100" label-position="left">
    <FormItem :label="$root.$t('message.publish.desc')">
      <Input
        :rows="4"
        type="textarea"
        v-model="comment"
        :placeholder="$root.$t('message.publish.inputDesc')"
      ></Input>
    </FormItem>
  </Form>
</template>
  ...
  methods: {
      ...
      api
        ...
        .catch(() => {
          this.$Message.warning(
            this.$root.$t('message.publish.error', {
    ...
    checkResult(id, timeoutValue) {
      const timer = setTimeout(() => {
        ...
        api
          .fetch('/dss/publishQuery', { projectVersionID: +id }, 'get')
          .then(res => {
            if (timeoutValue <= 10 * 60 * 1000) {
              if (
                ...
                this.$Message.success(
                  this.$root.$t('message.publish.success', {
                    name: this.currentProjectData.name
                  })
                )
                this.removePercent()
              ...
              clearTimeout(timer)
              this.$Message.warning(
                this.$root.$t('message.publish.timeout', {
                  name: this.currentProjectData.name
                })
              )
              this.removePercent()
