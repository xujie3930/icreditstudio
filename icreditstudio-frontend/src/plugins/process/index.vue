...
import { NODETYPE, NODEICON } from './service/nodeType'
...
export default {
  ...
  methods: {
    ...
    dblclickNode(index, args) {
      ...
      for (let i = 0; i < this.tabs.length; i++) {
        if (node.type == NODETYPE.SHELL) {
          if (
            this.tabs[i].node &&
            this.tabs[i].node.createTime === node.createTime &&
            this.tabs[i].node.key === node.key
          ) {
      ...
      // iframe打开的节点
      if (
        [NODETYPE.DISPLAY, NODETYPE.DASHBOARD, NODETYPE.QUALITIS].includes(
          node.type
        )
      ) {
    ...
    saveNode(args, node) {
      ...
      if (
        node.createTime !== currentNode.createTime &&
      ...
      if (
        ...
        node.resources[0].fileName === resource.fileName
      ) {
        // 已报保存过的直接替换，没有保存的首项追加
      ...
      this.$refs.process.map(item => {
        item.json.nodes.forEach(item => {
          if (
            item.createTime === currentNode.createTime &&
            item.key === currentNode.key
          ) {
...
<style lang="scss" src="./styles/process.scss"></style>
