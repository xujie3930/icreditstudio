package com.jinninghui.datasphere.icreditstudio.sparkx.common.utils
...
object Utils extends Logging {
  ...
  def exec(commandLine: List[String], maxWaitTime: Long): String = {
    ...
    IOUtils.closeQuietly(log)
    if (exitCode != 0) {
      throw new IllegalStateException(s"exec failed with exit code: $exitCode, ${lines.mkString(". ")}")
    }
    lines.mkString("\n")
  }
